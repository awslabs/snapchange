...
fn main() {
    ...
    let re = Regex::new(r"Input buffer: (0x[0-9A-Fa-f]+)").unwrap();
    ...
    let re = Regex::new(r"Size Addr: (0x[0-9A-Fa-f]+)").unwrap();
    let captures = re.captures(&vmlog).unwrap();
    let input_addr = &captures.get(1).unwrap().as_str();
    writeln!(w, "pub const INPUT_ADDR: u64 = {};", input_addr).unwrap();
}
